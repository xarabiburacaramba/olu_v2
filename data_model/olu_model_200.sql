...
-- Started on 2021-03-09 16:15:06
...
CREATE DATABASE olu_db WITH TEMPLATE = template0 ENCODING = 'UTF8' LC_COLLATE = 'Czech_Czechia.1250' LC_CTYPE = 'Czech_Czechia.1250';
...
--
-- TOC entry 246 (class 1259 OID 83128)
-- Name: olu_datasets_info; Type: VIEW; Schema: olu2; Owner: olu_admin
--

CREATE VIEW olu2.olu_datasets_info AS
 SELECT od.fid AS dataset_id,
    od.uri,
    om.origin_name,
    odt.type_name,
    om.valid_from,
    ...
    om.origin_type,
    om.column_names,
    metadata_theme.themes
   FROM olu2.origin_dataset od,
    olu2.origin_dataset_type odt,
    (olu2.origin_metadata om
     LEFT JOIN ( SELECT tab.metadata_id,
            json_agg(json_build_object('theme_uri', tab.theme_uri, 'theme_name', tab.theme_name, 'z_value', tab.z_value)) AS themes
           FROM ( SELECT omt.metadata_fid AS metadata_id,
                    omt.theme_fid AS theme_id,
                    omt.z_value,
                    ot.fid AS theme_id,
                    ot.theme_uri,
                    ot.theme_name
                   FROM (olu2.origin_metadata_to_theme omt
                     LEFT JOIN olu2.theme ot ON ((ot.fid = omt.theme_fid)))) tab(metadata_id, theme_id, z_value, theme_id_1, theme_uri, theme_name)
          GROUP BY tab.metadata_id) metadata_theme ON ((metadata_theme.metadata_id = om.fid)))
  WHERE ((od.metadata_fid = om.fid) AND (od.dataset_type = odt.type_id))
  ORDER BY od.fid;


ALTER TABLE olu2.olu_datasets_info OWNER TO olu_admin;
...
--
-- TOC entry 258 (class 1259 OID 83356)
-- Name: olu_in_original_layers; Type: VIEW; Schema: olu2; Owner: olu
--

CREATE VIEW olu2.olu_in_original_layers AS
 SELECT obj.fid,
    obj.geom,
    att.hilucs_id AS hilucs_value,
    att.atts,
    met.origin_name
   FROM olu2.olu_object obj,
    olu2.olu_attribute_set att,
    olu2.atts_to_object att2obj,
    olu2.origin_dataset ordat,
    olu2.origin_metadata met
  WHERE ((obj.fid = att2obj.object_fid) AND (att2obj.atts_fid = att.fid) AND (obj.dataset_fid = ordat.fid) AND (ordat.fid = att.dataset_fid) AND (ordat.metadata_fid = met.fid))
  ORDER BY obj.fid, obj.z_value;


ALTER TABLE olu2.olu_in_original_layers OWNER TO olu;
...
--
-- TOC entry 257 (class 1259 OID 83351)
-- Name: olu_objects_full_v3; Type: VIEW; Schema: olu2; Owner: olu
--

CREATE VIEW olu2.olu_objects_full_v3 AS
SELECT
    NULL::bigint AS fid,
    NULL::public.geometry(MultiPolygon,4326) AS geom,
    NULL::integer AS z_value,
    NULL::json AS attributes;


ALTER TABLE olu2.olu_objects_full_v3 OWNER TO olu;
...
--
-- TOC entry 3795 (class 2618 OID 83354)
-- Name: olu_objects_full_v3 _RETURN; Type: RULE; Schema: olu2; Owner: olu
--

CREATE OR REPLACE VIEW olu2.olu_objects_full_v3 AS
 SELECT obj.fid,
    obj.geom,
    obj.z_value,
    json_agg(json_build_object('atribute_origin', atributy.origin_name, 'dataset', atributy.dataset_uri, 'HILUCS', atributy.hilucs_value, 'other_attributes', atributy.atts)) AS attributes
   FROM (olu2.olu_object obj
     LEFT JOIN ( SELECT a2o.object_fid,
            aov.type_name AS origin_name,
            od.uri AS dataset_uri,
            oa.hilucs_id AS hilucs_value,
            oa.atts
           FROM olu2.atts_to_object a2o,
            olu2.olu_attribute_set oa,
            olu2.origin_dataset od,
            olu2.attribute_origin_type aov
          WHERE ((a2o.atts_fid = oa.fid) AND (oa.dataset_fid = od.fid) AND (a2o.atts_origin = aov.type_id))) atributy ON ((atributy.object_fid = obj.fid)))
  GROUP BY obj.fid
  ORDER BY obj.z_value;
...
-- Completed on 2021-03-09 16:15:06
